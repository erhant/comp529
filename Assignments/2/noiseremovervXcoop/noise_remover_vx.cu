...
namespace cg = cooperative_groups; // use like cg::sync(grid) or cg::this_grid() etc.
...
__global__ void coop_kernel_redux(unsigned char* image, int width, int height, int n_pixels, float lambda, float* diff_coef, float* sum, float* sum2) {
	int j = blockIdx.x * blockDim.x + threadIdx.x;
	int i = blockIdx.y * blockDim.y + threadIdx.y;
	long k = i * width + j;
	float image_k;
	float north, south, west, east, diff_coef_k; // used in two parts
	cg::grid_group thisGrid = cg::this_grid();

	if (j >= 0 && j < width && i >= 0 && i < height) {
		image_k = image[k]; // 1 global access
		atomicAdd(&sum[0], image_k); // 1 global access
		atomicAdd(&sum2[0], image_k * image_k); // 1 global access
	}

	cg::sync(thisGrid); // sync device

	float mean = sum[0] / n_pixels;  // 1 global access
	float variance = (sum2[0] / n_pixels) - mean * mean;  // 1 global access
	float stdDev = variance / (mean * mean); 
	...
	if (j > 0 && j < width - 1 && i > 0 && i < height - 1) {
		float divergence = diff_coef_k * north + diff_coef[k + width] * south + diff_coef_k * west + diff_coef[k + 1] * east; // --- 7 floating point arithmetic operations, 6 global accesses
		image[k] = image[k] + 0.25 * lambda * divergence; // --- 3 floating point arithmetic operations, 2 global access (read and write)
	}
}

/*
__global__ void coop_kernel_redux(unsigned char* image, float* reduction_arr, int width, int height, int n_pixels) {
	extern __shared__ float sdata[]; // size of this shared memory will be given as a kernel launch parameter
	unsigned int tid, i, j, k, blockDim1D;
	j = blockIdx.x * blockDim.x + threadIdx.x;
	i = blockIdx.y * blockDim.y + threadIdx.y;

	cg::grid thisGrid = cg::this_grid();
	// Reduce (level 1) image (size n) -> reduction_array (size blockDim.x * blockDim.y)
	blockDim1D = blockDim.x * blockDim.y; // convert to 1D dimension of block
	k = i * width + j; // convert to 1D index on image
	tid = threadIdx.y * blockDim.x + threadIdx.x; // convert to 1D index within block
	float image_k = image[k];
	sdata[tid] = image_k; // each thread loads one pixel of image
	sdata[tid+blockDim1D] = image_k * image_k; // for sum2
	__syncthreads(); // wait for the load to complete

	for (unsigned int s = blockDim1D/2; s>0; s>>=1) {
		if (tid < s) { // indexes 2k, 4k, 8k etc.
			sdata[tid] += sdata[tid + s]; // sum
			sdata[blockDim1D + tid] += sdata[blockDim1D + tid + s]; // sum2
		}
		__syncthreads();
	}

	if (tid == 0) {
		reduction_arr[blockIdx.y * blockDim.x + blockDim.x] = sdata[0]; // write to block index in 1D dimensions, sum1
		reduction_arr[blockDim1D + blockIdx.y * blockDim.x + blockDim.x] = sdata[blockDim1D]; // sum2
	}

	cg::sync(thisGrid); // wait for all blocks to complete

	// Reduce (level 2) reduction_array (size blockDim.x * blockDim.y) -> 
	// todo
	float sum = 0;
	for (int r_i = 0; r_i < blockDim1D; r_i++) {
	...
	// Statistics
	if (tid == 0) {
		// Only one thread per block would be enough for this step
 ...
 int main(int argc, char *argv[]) {
	...
	for(int ac=1;ac<argc;ac++) {
		if(MATCH("-i")) {
			...
			compute_kernel_type = atoi(argv[++ac]);
		 } else {
			printf("Usage: %s [-i < filename>] [-iter <n_iter>] [-l <lambda>] [-o <outputfilename>] [-s 1|2|3]\n",argv[0]);
			return(-1);
		 }
	}
	time_2 = get_time();

	// (HOST) Extra: Check device properties
	// check GPU architectures
	cudaDeviceProp prop;
	CUDA_CHECK(cudaGetDeviceProperties(&prop, 0));
	if (!prop.cooperativeLaunch) {
        fprintf(stderr, "Cooperative Launch is not available on GPU 0.");
        exit(1);
	}
	// calculate occupancy for grid sync
	int numOfsms = prop.multiProcessorCount;
	int numblockspersm;
	CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numblockspersm, coop_kernel_redux, DIM_THREAD_BLOCK_X * DIM_THREAD_BLOCK_Y, 0));
	printf("Max occupancy: %d", numblockspersm * numOfsms);
	...
	// (HOST) Extra: Calculate block and grid parameters, specify coop thread parameters.	
	...
	int sMemSize = 0; // block.x * block.y * 2; // shared memory size for reduction
	void* kernelArgs[] = {
		(void *)&d_image, 
		(void *)&width,
		(void *)&height,
	...
	for (int iter = 0; iter < n_iter; iter++) {
		CUDA_CHECK(cudaLaunchCooperativeKernel((void *) coop_kernel_redux, grid, block, kernelArgs, sMemSize, stream))
	}
