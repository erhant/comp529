...
int main(int argc, char **argv) {
	...
	*/
	...
	int i, j, k, t, myRow, rpw, avgnnzpw;
	...
	if (taskid == master) {
		...
		// Calculate nonzero per worker (WE ASSUME nnz >> number of processors and nnz are rather distributed over the matrix)
		avgnnzpw = int(floor(float(matrix.nnz) / float(numtasks))); // rows per worker

		// Construct recieve counts and myRows
		int nnz_tmp, rpw_tmp;
		j = 0;
		...
		// Broadcast the recieve counts, which also hold the rpw for each worker
		MPI_Bcast(recieveCounts, numtasks, MPI_INT, master, MPI_COMM_WORLD);

		// Calculate myRow array and your own myRow
		//int* workerRows = new int[numtasks];
		//workerRows[0]=0;
		//for (i = 0; i<numtasks-1; i++) {
		//	workerRows[i+1] = recieveCounts[i] + workerRows[i];
		//}
		myRow = offsets[numtasks-1];
		...
		for (i = 0; i < numtasks - 1; ++i) {
			...
			MPI_Isend(&(matrix.csrRowPtr[offsets[i]]), recieveCounts[i]+1, MPI_INT, i, RO_MSG_TAG, MPI_COMM_WORLD, &sendRequests[3*i]);
			// Send column indices to slave
			MPI_Isend(&(matrix.csrColIdx[matrix.csrRowPtr[offsets[i]]]), matrix.csrRowPtr[offsets[i+1]] - matrix.csrRowPtr[offsets[i]], MPI_INT, i, CI_MSG_TAG, MPI_COMM_WORLD, &sendRequests[3*i+1]);			
			// Send column values to slave
			MPI_Isend(&(matrix.csrVal[matrix.csrRowPtr[offsets[i]]]), matrix.csrRowPtr[offsets[i+1]] - matrix.csrRowPtr[offsets[i]], MPI_DOUBLE, i, CV_MSG_TAG, MPI_COMM_WORLD, &sendRequests[3*i+2]);
		}
		...
		// Get rpw from master
		MPI_Bcast(recieveCounts, numtasks, MPI_INT, master, MPI_COMM_WORLD);
		rpw = recieveCounts[taskid];

		// Calculate myRow from the recieveCounts
		myRow = 0;
		for (i = 0; i<taskid; i++) {
			myRow += recieveCounts[i];
		}
