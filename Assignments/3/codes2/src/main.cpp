...
#define MAX_NUM_OF_PROCESSORS 16
...
int main(int argc, char **argv) {
	...
	*/
	...
	int numthreads = MAX_NUM_OF_PROCESSORS / numtasks; //  MPI tasks x OpenMP threads = MAX_NUM_OF_PROCESSORS, as per request in the assignment.
	...
	if (taskid == master) {
		/*** MASTER ***/
		printf("Utilizing OpenMP with %d threads per processor.\n", numthreads);
		...
		offsets[0] = 0;
		// we can not parallelize this, it has dependency
		...
		for (t = 0; t < time_steps; ++t) {
			...
			#pragma omp parallel for num_threads(numthreads) if(numthreads > 1) private(j, k)
			for (i = 0; i < matrix.m - myRow; ++i) { 
				k = matrix.csrRowPtr[myRow + i]; // offset k to the starting point of this row
				result[myRow + i] = 0.0;
				for (j = 0; j < matrix.csrRowPtr[myRow + i + 1] - matrix.csrRowPtr[myRow + i]; ++j) {
					#if VERBOSE == 2
					printf("Task %d, k+j = %d\n", taskid, k+j);
					#endif
					result[myRow + i] += matrix.csrVal[k + j] * rhs[matrix.csrColIdx[k + j]]; // in-row offset is done by j
				}
			}
		...
		for (t = 0; t < time_steps; ++t) {
			#pragma omp parallel for num_threads(numthreads) if(numthreads > 1) private(j, k)
			for (i = 0; i < rpw; ++i) {
				k = csrRowPtr[i] - csrRowPtr[0]; // offset k to the starting point of this row
				result[myRow + i] = 0.0;
				for (j = 0; j < csrRowPtr[i+1] - csrRowPtr[i]; ++j) {
					#if VERBOSE == 2
					printf("Task %d, k+j = %d\n", taskid, k+j);
					#endif
					result[myRow + i] += csrVal[k + j] * rhs[csrColIdx[k + j]]; // in-row offset is done by j
				}
			}
